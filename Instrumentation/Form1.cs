        ...
        public Form1()
        {
            ...
            listBox1.SelectedIndexChanged += (s, a) =>
            {
                if (cont == 2)
                {
                    ...
                    {
                        //18
                        richTextBox1.Text = "По умолчанию глиссандо выполняется с помощью передвижения пальца левой руки вдоль прижимаемой им струн \r\rПриём игры — арко";
            ...
            listBox1.SelectedIndexChanged += (s, a) =>
            {
                if (cont == 5)
                {
                    ...
                    else if (listBox1.SelectedIndex < 38)
                    {
                        ...
                        {
                            richTextBox1.Text = "Атака звуков острая и акцентированная.Между звуками есть пауза. Смы  чок не отрывается от струн. Каждый следующий звук исполняется другим направлением ве  дения смычка(поочерёд  но вниз и вверх)\r\rПриём игры — арко";
            ...
            listBox1.SelectedIndexChanged += (s, a) =>
            {
                if (cont == 11)
                {
                    ...
                    {
                        //10
                        richTextBox1.Text = "Флажолет — это приём игры, который исполняется лёгким прикосновением пальца к струне в точке её деления на несколько равных отрезков. Прикосновение само по себе не производит звукоизвлечение и в данном случае осуществляется вместе с щипком/ударом. Сразу после звукоизвлечения прикасающийся палец должен прервать контакт со струной. В результате извлекается какойлибо обертон. Кроме того, флажолетом называется сам извлекаемый обертон\r\rПрикосновение к струне в точке её деления на два равных отрезка приводит к извлечению октавного обертона (и звучанию струны на октаву выше). Прикосновение в одной из точек деления на три равных отрезка приводит к извлечению квинтового обертона (и звучанию струны на дуодециму выше) и т. д.\r\rНатуральный флажолет исполняется на открытой струне";
                        if (listBox1.SelectedIndex < 20)
                        {
                            richTextBox1.Text = null;
                            pictureBox1.Image = null;
